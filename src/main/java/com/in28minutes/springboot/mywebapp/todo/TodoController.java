...
public class TodoController {
	...
	public String listAllTodos(Model model) {
		model.addAttribute("todos", todoService.findByUserName((String)model.getAttribute("name")));
		return "listTodos";
	}
	...
	public String addNewTodo(@Valid Todo todo, BindingResult bindingResult, Model model) {
		if (bindingResult.hasErrors()) {
			...
			boolean result = todoService.addNewTodo(todo);
			if (result) {
				model.addAttribute("success", "Todo was added successfully! Add another?");
				model.addAttribute("todo", null);
			} else
				model.addAttribute("error", "* Please fill the form properly then try again");
		}
		return "todo";
	}
